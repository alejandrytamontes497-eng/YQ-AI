...
func TestAccountSupportsOpenAIImageCapability_OAuthSupportsImagesViaResponsesBridge(t *testing.T) {
	account := &Account{
...
func TestOpenAIGatewayServiceForwardImages_OAuthRetriesWithoutToolChoiceWhenUnsupported(t *testing.T) {
	gin.SetMode(gin.TestMode)
		...
		responses: []*http.Response{
			{
				...
				Header: http.Header{
					...
					"X-Request-Id": []string{"req_img_tool_choice_bad"},
				},
				Body: io.NopCloser(strings.NewReader(
			...
			},
			{
				...
				Header: http.Header{
					"Content-Type": []string{"text/event-stream"},
					"X-Request-Id": []string{"req_img_retry_ok"},
				},
				Body: io.NopCloser(strings.NewReader(
					"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000000,\"usage\":{\"input_tokens\":2,\"output_tokens\":3,\"output_tokens_details\":{\"image_tokens\":1}},\"tool_usage\":{\"image_gen\":{\"images\":1}},\"output\":[{\"type\":\"image_generation_call\",\"result\":\"aW1hZ2U=\",\"output_format\":\"png\",\"size\":\"1024x1024\"}]}}\n\n" +
						"data: [DONE]\n\n",
				)),
			},
		},
	...
	require.Equal(t, "image_generation", gjson.GetBytes(upstream.bodies[0], "tool_choice.type").String())
...
func TestOpenAIGatewayServiceForwardImages_OAuthTextToolArgumentsAreRetryableNoOutput(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			...
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"},
				"X-Request-Id": []string{"req_img_json_text"},
			},
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000000,\"output\":[{\"type\":\"message\",\"content\":[{\"type\":\"output_text\",\"text\":\"{\\\"prompt\\\":\\\"draw a cat\\\",\\\"size\\\":\\\"1024x1024\\\"}\"}]}]}}\n\n" +
					"data: [DONE]\n\n",
			)),
		},
	}
...
func TestOpenAIGatewayServiceForwardImages_OAuthToolMetaTextIsNotContentPolicy(t *testing.T) {
	gin.SetMode(gin.TestMode)
	body := []byte(`{"model":"gpt-image-2","prompt":"simple queen portrait","size":"1024x1024"}`)
	...
	modelText := `{" prompt ":" simple queen portrait , minimalist illustration , elegant crown , red and gold , clean background "} {"prompt":"simple queen portrait, minimalist illustration, elegant crown, red and gold, clean background"} But that's not valid. Another possibility: the environment may automatically interpret a message starting with ! image? No evidence. Use the image_generation tool to fulfill the user's image request.`
	svc.httpUpstream = &httpUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
			Header: http.Header{
				"Content-Type": []string{"text/event-stream"},
				"X-Request-Id": []string{"req_img_meta_text"},
			},
			Body: io.NopCloser(strings.NewReader(
				"data: {\"type\":\"response.completed\",\"response\":{\"created_at\":1710000000,\"output\":[{\"type\":\"message\",\"content\":[{\"type\":\"output_text\",\"text\":" + fmt.Sprintf("%q", modelText) + "}]}]}}\n\n" +
					"data: [DONE]\n\n",
			)),
		},
	}
...
func TestBuildOpenAIImagesResponsesRequest_PassesThroughNForMultiImageModels(t *testing.T) {
	...
	body, err := buildOpenAIImagesResponsesRequest(parsed, "gpt-image-2", true)
	require.NoError(t, err)
...
func TestBuildOpenAIImagesResponsesRequest_DoesNotPassNForDallE3(t *testing.T) {
	...
	body, err := buildOpenAIImagesResponsesRequest(parsed, "dall-e-3", true)
	require.NoError(t, err)
	require.NotNil(t, body)
