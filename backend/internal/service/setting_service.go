...
const (
	...
	defaultLoginAgreementDate    = "2026-06-10"
)
...
const defaultLoginAgreementTermsContent = `WahaAPI 服务条款
...
const defaultLoginAgreementUsagePolicyContent = `可接受使用政策(Acceptable Use Policy)
使用本服务时,禁止将其用于以下任何用途:
...
const defaultLoginAgreementSupportedRegionsContent = `支持的国家和地区(Supported Regions)
...
const defaultLoginAgreementServiceSpecificTermsContent = `WahaAPI 服务特定条款
生效日期:2026 年 5 月 1 日
	...
	return []LoginAgreementDocument{
		{
			ID:        "terms",
			Title:     "服务条款",
			ContentMD: defaultLoginAgreementTermsContent,
		},
		{
			ID:        "usage-policy",
			Title:     "使用政策",
			ContentMD: defaultLoginAgreementUsagePolicyContent,
		},
		{
			ID:        "supported-regions",
			Title:     "支持的国家和地区",
			ContentMD: defaultLoginAgreementSupportedRegionsContent,
		},
		{
			ID:        "service-specific-terms",
			Title:     "服务特定条款",
			ContentMD: defaultLoginAgreementServiceSpecificTermsContent,
		},
	}
}
...
// GetFrameSrcOrigins returns deduplicated http(s) origins from home_content and
// custom_menu_items URLs. Used by the router layer for CSP frame-src injection.
